...
// First read existing cities to get their Chinese names
const oldContent = fs.readFileSync('src/data/cities.ts', 'utf8');
const nameMap = {};
const matchArray = oldContent.match(/export const BUILTIN_CITIES.*?=\s*(\[[\s\S]*?\]);/);
if (matchArray && matchArray[1]) {
  try {
    // using eval to parse the array since it might not be strict JSON
    const oldCities = eval(matchArray[1]);
    for (const c of oldCities) {
      nameMap[c.countryIso3] = c.name; // map ISO3 to Chinese capital name
    }
  } catch(e) {
    console.error("Failed to parse old cities", e);
  }
}
  ...
  res.on('end', () => {
    ...
    for (const c of countries) {
      if (c.capital && c.capital.length > 0 && c.latlng && c.latlng.length === 2) {
        ...
        cities.push({
          name: nameMap[c.cca3] || c.capital[0], // use existing chinese name if available, else english
          lat: lat,
          lng: lng,
          isCapital: true,
          countryIso3: c.cca3
...
`;
    ...
    console.log('Done generating src/data/cities.ts with ' + cities.length + ' capitals, preserved ' + Object.keys(nameMap).length + ' Chinese names.');
  });
});
