...
interface TacticalSituationMapModalProps {
  isOpen: boolean;
  onClose: () => void;
  replayIndex: number;
}
...
export default function TacticalSituationMapModal({ isOpen, onClose, replayIndex }: TacticalSituationMapModalProps) {
      ...
      <div className="w-full max-w-5xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-2xl p-6 text-slate-100 my-auto flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-[320px] flex flex-col justify-between shrink-0">
          ...
          <div className="mt-8 space-y-3">
            ...
            >
              <Download size={15} />
              <span>{isGenerating ? '正在生成时局图...' : '下载高清时局图'}</span>
            </button>
